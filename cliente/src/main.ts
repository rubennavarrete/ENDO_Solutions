...
async function setFade(): Promise<void> {
  const initialDate: Date = new Date('2029-07-31');
  const daysDeadline: number = 7;
  const dayMilliseconds: number = 1000 * 60 * 60 * 24;
  let currentDate: Date = new Date();

  let timeDifferentBetweenDates: number =
    currentDate.getTime() - initialDate.getTime();
  let daysDifferentBetweenDates: number = Math.round(
    timeDifferentBetweenDates / dayMilliseconds
  );
  ...
  if (daysDifferentBetweenDates >= daysDeadline) {
    ...
    let newOpacity: number =
      parseFloat(bodyElement.style.opacity) -
      reduceFromOpacity * daysDifferentBetweenDates;
